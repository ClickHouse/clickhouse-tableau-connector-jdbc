...
Starting version `v0.3.2` The usage of these two fields were removed due to vulnerabilities and security concerns (as detailed in [Tableau's official documentation](https://tableau.github.io/connector-plugin-sdk/docs/security-considerations#freeform-driver-properties)). Please add your parameters through a [JDBC properties file](https://help.tableau.com/current/pro/desktop/en-us/connect_customize.htm#use-a-properties-file-to-customize-a-jdbc-connection)

- **Custom Connection Parameters**. By default, `socket_timeout` is already specified, this parameter may need to be changed if some extracts are updated for a very long time. The value of this parameter is specified in milliseconds. The rest of the parameters can be found [here](https://github.com/ClickHouse/clickhouse-jdbc/blob/master/clickhouse-client/src/main/java/com/clickhouse/client/config/ClickHouseClientOption.java).
